...
**A multi-agent tourist AI assistant for grounded trip planning anywhere.** Hodari helps travelers and football fans plan personalized itineraries — *"4 hours, $60, vegetarian in Kigali"* or *"vegetarian near Camp Nou"* — through a conversational interface backed by map data, and renders plans on an interactive map with feasible walking routes.

> Hodari (Swahili: *brave / capable*) turns free time in any city into a concrete plan: where to eat, what to see, and how to get there.
...
- **Live local app path:** `client/` → Next API routes → FastAPI `backend/` on `:8001` → planner/researcher/itinerary modules.
- **ADK architecture path:** `agents/` contains the richer Google ADK/MCP implementation described below and can be used as the target production agent runtime.

Four agents run conceptually in one process with in-process routing. The Orchestrator owns session state and streams the final answer to the client over SSE.
...
│   ├── Dockerfile
│   └── .env.example
├── backend/                    # Current FastAPI backend used by the Next.js client locally
│   ├── agents/                 # planner.py · researcher.py · itinerary.py · orchestrator.py
│   ├── routers/                # /agent/turn · /agent/voice
│   ├── tools/                  # Mongo/session/preference + Google Places live search helper
│   └── tests/
...
Then open **http://localhost:3000** and try: *"4 hrs, $60, vegetarian in Kigali"* or *"vegetarian near Camp Nou"*.
...
### Current FastAPI local path

The checked-in Next.js client currently defaults to `BACKEND_BASE_URL=http://127.0.0.1:8001`, so the simplest local full-stack loop is:
...
If `GOOGLE_MAPS_API_KEY` or `MAPS_API_KEY` is set in `backend/.env`, the FastAPI researcher uses Google Places Text Search globally. Without a key, it falls back to useful local demo candidates for known places such as Kigali/Rwanda, Camp Nou/Barcelona, MetLife, and generic current-location plans.
