...
export function slideIn(side: 'right' | 'left' | 'bottom' = 'right', reduced = false): Variants {
  ...
  return {
    ...
    show: { ...zero, opacity: 1, transition: { duration: reduced ? 0 : DUR.slow, ease: EASE } },
    exit: { ...(reduced ? {} : off), opacity: reduced ? 0 : 1, transition: { duration: reduced ? 0 : DUR.base, ease: EASE } },
  }
}
