...
EXPLORER_INSTRUCTION = """You are the Explorer for Hodari, a global tourist AI assistant.
...
3. Call search_places for each subtask in the plan — use it to fill gaps not covered by
   ...
   - For "near me" requests, always bias the search with the user's current GPS coordinates.
...
CRITICAL:
- Only return real places from search_places or the verified local place database.
- Never invent placeholder names like "Nearby Food Hub", "Nearby Local Highlight", "Suggested place",
  or "Local Restaurant".
- If there are no real place results, return an empty JSON array rather than fake places.
- Preserve real business names, addresses, ratings, price level, hours, and coordinates.
...
def create_explorer_agent(*, mode: str | None = None) -> LlmAgent:
    return LlmAgent(
